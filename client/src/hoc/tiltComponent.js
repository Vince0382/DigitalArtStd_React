...
class TiltComponent extends Component {
  ...
  update(e) {
    ...
    this.setState(prevState => ({
      style: {
        ...prevState.style,
        transform: `translateZ(${this.settings.perspective}px) perspective(${this.settings.perspective}px) rotateX(${this.settings.axis === 'x' ? 0 : values.tiltY}deg) rotateY(${this.settings.axis === 'y' ? 0 : values.tiltX}deg) scale3d(${this.settings.scale}, ${this.settings.scale}, ${this.settings.scale})`
      }
    }))
    this.updateCall = null
  }
